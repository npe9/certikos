...
(*              Layers of PM: PCurID                                   *)
(*                                                                     *)
(*          Provide the abstraction of current thread id               *)
(*                                                                     *)
...
Section WITHMEM.
  ...
  Global Instance pcurid_data_ops : CompatDataOps RData :=
    {
      empty_data := init_adt;
  ...
  Section Property_Abstract_Data.
    ...
    Global Instance pcurid_data_prf : CompatData RData.
    Proof.
      constructor.
  ...
  (** * Layer Definition *)
  Definition pcurid_fresh : compatlayer (cdata RData) :=
    get_curid ↦ gensem get_curid_spec
              ⊕ set_curid ↦ gensem set_curid_spec.

  Definition pcurid_passthrough : compatlayer (cdata RData) :=
  ...
  Definition pcurid : compatlayer (cdata RData) := pcurid_fresh ⊕ pcurid_passthrough.

  (*Definition pcurid_impl : compatlayer (cdata RData) :=
    kctxt_new ↦ dnew_compatsem
  ...
  Definition pcurid_rest : compatlayer (cdata RData) :=
    get_curid ↦ gensem get_curid_spec
  ...
  Lemma pcurid_impl_eq : pcurid ≡ pcurid_impl ⊕ pcurid_rest.
  Proof. reflexivity. Qed.

  Definition semantics := LAsm.Lsemantics pcurid.*)
