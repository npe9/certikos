...
Section WITHMEM.
  ...
  Global Instance preinit_data_ops : CompatDataOps RData :=
    {
      empty_data := init_adt;
  ...
  Section Property_Abstract_Data.
    ...
    Global Instance preinit_data_prf : CompatData RData.
    Proof.
      constructor.
  ...
  Section INV.
    ...
    Global Instance clearCR2_fst_inv: PreservesInvariants clearCR2_fst_spec.
    Proof.
      preserves_invariants_simpl low_level_invariant high_level_invariant.
    Qed.
  ...
  Definition exec_loadex {F V} := exec_loadex0 (F := F) (V := V).

  Definition exec_storeex {F V} := exec_storeex0 (F := F) (V := V).
  ...
  (** * Layer Definition *)
  Definition preinit : compatlayer (cdata RData) :=
    vmxinfo_get ↦ gensem vmxinfo_get_spec
                ...
                ⊕ boot_loader ↦ gensem bootloader_spec
