...
Section Load_Store.
  ...
  Section GE.
    ...
    Definition exec_loadex2 (chunk: memory_chunk) (m: mwd HDATAOps)
      ...
      match (eval_addrmode ge a rs) with
        ...
        | Vint adr => 
          match (ihost adt, pg adt, ikern adt) with
            ...
            | (false, _, _) => exec_guest_intel_load1 ge adr chunk m rs rd                                         
            | _ => Stuck
          end
        | _ => Stuck
      end.

      Definition exec_storeex2 (chunk: memory_chunk) (m: mwd HDATAOps)
        ...
        match  (eval_addrmode ge a rs) with
          ...
          | Vint adr => 
            match (ihost adt, pg adt, ikern adt) with
              ...
              | (false, _, _) => exec_guest_intel_store1 (flatmem_store:= flatmem_store) ge adr chunk m rs rd destroyed                                         
              | _ => Stuck
            end
          | _ => Stuck
        end.
      ...
      Lemma exec_loadex2_high_level_invariant:
      ...
      Lemma exec_loadex2_low_level_invariant:
        forall adr chunk m rs rd rs' m',
          exec_loadex2 chunk m adr rs rd = Next rs' m' ->
          AsmX.asm_invariant ge rs m ->
      ...
      Lemma exec_storeex2_high_level_invariant:
      ...
      Lemma exec_storeex2_low_level_invariant:
        forall adr chunk m rs rd des rs' m',
          exec_storeex2 chunk m adr rs rd des = Next rs' m' ->
          AsmX.asm_invariant ge rs m ->
          CompatData.low_level_invariant (Mem.nextblock m) (snd m) ->
  ...
  Global Instance load_accessor_prf2:
    LoadAccessor _ (@exec_loadex2).
  Proof.
    constructor.
    {
      unfold exec_loadex2; intros.
  ...
  Global Instance store_accessor_prf2:
    StoreAccessor _ (@exec_storeex2).
  Proof.
    constructor.
    {
      unfold exec_storeex2. intros.
      erewrite AsmX.eval_addrmode_symbols_preserved; eauto.
      destruct (eval_addrmode ge1 a rs); try reflexivity.
      - destruct (ihost (snd m)); try reflexivity.
        destruct (pg (snd m)); destruct (ikern (snd m)); try reflexivity.
...
Section Load_Store_Refinement.
  ...
  Section Load_Store2.
    ...
    Notation hStore := (fun F V => exec_storeex2 (F:=F) (V:=V) (flatmem_store:= hflatmem_store)).
    Notation lStore := (fun F V => exec_storeex2 (F:=F) (V:=V) (flatmem_store:= lflatmem_store)).
    ...
    Proof.
      ...
      exploit relate_impl_iflags_eq; eauto. inversion 1.
      pose proof (stencil_matches_preserves_trans _ _ _ _ _ _ _ H0 H) as Hpre.
      unfold exec_storeex2 in *. simpl in *. 
      rewrite <- ihost_eq, <- ikern_eq, <- pg_eq.
