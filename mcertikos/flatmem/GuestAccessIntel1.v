    ...
    Section Load_Store1.
      ...
      Lemma exec_guest_intel_load1_high_level_invariant:
        forall adr chunk m rs rd rs' m',
          exec_guest_intel_load1 adr chunk m rs rd = Next rs' m' ->
      ...
      Lemma exec_guest_intel_store1_high_level_invariant:
        forall adr chunk m rs rd rs' ds m',
          exec_guest_intel_store1 adr chunk m rs rd ds = Next rs' m' ->
          high_level_invariant (snd m) ->
          high_level_invariant (snd m').
      Proof.
  ...
  Section General.
    ...
    Lemma guest_intel_load_correct1:
      forall {F V} (ge1 ge2: Genv.t F V) m1 m1' m2 d1 d2 d1' rs1 rs2 rs1' f s rd t i,
        exec_guest_intel_load1 ge1 i t (m1, d1) rs1 rd = Next rs1' (m1', d1')
        -> MatchPrimcallStates (one_crel HDATAOps LDATAOps) s f rs1 m1 d1 rs2 m2 d2
        -> stencil_matches s ge1
        -> stencil_matches s ge2
        -> exists r'0 m2' d2',
             exec_guest_intel_load1 ge2 i t (m2, d2) rs2 rd = Next r'0 (m2', d2')
    ...
    Lemma guest_intel_store_correct1:
      forall {F V} (ge1 ge2: Genv.t F V) m1 m1' m2 d1 d2 d1' rs1 rs2 rs1' f s rd t i ds,
        exec_guest_intel_store1 (flatmem_store:= hflatmem_store) ge1 i t (m1, d1) rs1 rd ds = Next rs1' (m1', d1')
        -> MatchPrimcallStates (one_crel HDATAOps LDATAOps) s f rs1 m1 d1 rs2 m2 d2
        -> stencil_matches s ge1
        -> stencil_matches s ge2
        -> exists r'0 m2' d2',
             exec_guest_intel_store1 (flatmem_store:= lflatmem_store) ge2 i t (m2, d2) rs2 rd ds = Next r'0 (m2', d2')
             /\ MatchPrimcallStates (one_crel HDATAOps LDATAOps) s f rs1' m1' d1' r'0 m2' d2'.
    Proof.
      intros. eapply guest_intel_correct1; eauto.
