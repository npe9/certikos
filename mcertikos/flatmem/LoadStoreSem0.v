...
Require Import GuestAccessIntel0.
...
Section Load_Store.
  ...
  Section GE.
      ...
      match (eval_addrmode ge a rs) with
        ...
        | Vint adr => 
          match (ihost adt, pg adt) with
            ...
            | (false, _) => exec_guest_intel_load0 ge adr chunk m rs rd                                         
            | _ => Stuck
          end
        | _ => Stuck
      end.
      ...
      match  (eval_addrmode ge a rs) with
        ...
        | Vint adr => 
          match (ihost adt, pg adt) with
            ...
            | (false, _) => exec_guest_intel_store0 ge adr chunk m rs rd destroyed                                         
            | _ => Stuck
          end
        | _ => Stuck
      end.
    ...
    Proof.
      ...
      - destruct (ihost (snd m)); try discriminate.
        ...
        + eapply exec_guest_intel_load0_high_level_invariant; eauto.
    ...
    Proof.
      ...
      - destruct (ihost (snd m)); try discriminate.
        ...
        + intros; eapply exec_guest_intel_load0_asm_invariant; eauto.
      - destruct (ikern (snd m)); try discriminate.
        destruct (ihost (snd m)); try discriminate.
    ...
    Proof.
      ...
      - destruct (ihost (snd m)); try discriminate.
        ...
        + intros; eapply exec_guest_intel_load0_low_level_invariant; eauto.
      - destruct (ikern (snd m)); try discriminate.
        destruct (ihost (snd m)); try discriminate.
        unfold Asm.exec_load. 
    ...
    Proof.
      ...
      - destruct (ihost (snd m)); try discriminate.
        ...
        + eapply exec_guest_intel_store0_high_level_invariant; eauto.
      - destruct (ikern (snd m)); try discriminate.
        destruct (ihost (snd m)); try discriminate.
    ...
    Proof.
      ...
      - destruct (ihost (snd m)); try discriminate.
        ...
        + intros; eapply exec_guest_intel_store0_asm_invariant; eauto.
      - destruct (ikern (snd m)); try discriminate.
        destruct (ihost (snd m)); try discriminate.
    ...
    Proof.
      ...
      - destruct (ihost (snd m)); try discriminate.
        ...
        + intros; eapply exec_guest_intel_store0_low_level_invariant; eauto.
      - destruct (ikern (snd m)); try discriminate.
        destruct (ihost (snd m)); try discriminate.
  ...
  Context `{TrapinfoSetInvariant (Obs:=Obs) (data_ops:= data_ops)}.
        ...
        + unfold exec_host_store0.
          ...
          * destruct (Genv.find_symbol ge1 FlatMem_LOC); try reflexivity.
          ...
          (** destruct (Mem.loadv Mint32 m
            ...
            erewrite AsmX.eval_addrmode_symbols_preserved; eauto.*)
        + eapply exec_guest_intel_accessor1_eq; eauto.
          unfold store_accessor0, Asm.exec_store. intros.
          erewrite AsmX.eval_addrmode_symbols_preserved; eauto.
