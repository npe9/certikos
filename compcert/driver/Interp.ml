...
open AST
open Integers
(** [CompCertX:test-compcert-param-memory]: we used to depend on the memory model.
    We now explicitly refer to its concrete implementation. *)
open Values
open Globalenvs
open Events
open Ctypes
...
(** [CompCertX:test-compcert-param-memory] We now explicitly refer to the concrete implementation
    of the memory model. *)
...
let compare_state s1 s2 =
  ...
  let c = P.compare (mem_state s1).Memimpl.nextblock (mem_state s2).Memimpl.nextblock in
...
module StateSet =
  Set.Make(struct
(** [CompCertX:test-compcert-param-memory] [state] now depends on the [mem] type.
  We have to instantiate it here with the concrete implementation. *)
...
let rec purge_seen nextblock seen =
  ...
  | None -> seen
  | Some((s, w) as sw) ->
      if P.le (mem_state s).Memimpl.nextblock nextblock
...
(** [CompCertX:test-compcert-param-extcall] We separated EF_builtin from EF_extcall in the formalization. Let us not do the same here. *)
let do_builtin_function = do_external_function
...
(** [CompCertX:test-compcert-param-memory] We use the version already instantiated with
   the concrete implementation of the memory model. *)
(** [CompCertX:test-compcert-param-extcall] We separate EF_builtin from EF_extcall. *)
    let l = CexecImpl.step_expr ge do_external_function do_builtin_function do_inline_assembly e w k a m in
...
let do_step p prog ge time (s, w) =
  ...
  | None ->
(** [CompCertX:test-compcert-param-extcall] We separate EF_builtin from EF_extcall. *)
      let l = CexecImpl.do_step ge do_external_function do_builtin_function do_inline_assembly w s in
...
module PrioQueue = struct
  ...
  let higher_prio (time1, (s1, w1)) (time2, (s2, w2)) =
    P.lt (mem_state s1).Memimpl.nextblock (mem_state s2).Memimpl.nextblock
...
let execute prog =
  ...
  | Some prog1 ->
      ...
      match GlobalenvsImpl.init_mem wprog with
      | None ->
