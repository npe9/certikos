...
Lemma do_volatile_store_sound:
  ...
  volatile_store (fun _ => True) ge chunk m b ofs v t m' /\ possible_trace w t w'.
Proof.
...
Hypothesis do_external_function_sound:
  ...
  external_functions_sem id sg (fun _ => True) ge vargs m t vres m' /\ possible_trace w t w'.

Hypothesis do_external_function_complete:
  forall id sg ge vargs m t vres m' w w',
  external_functions_sem id sg (fun _ => True) ge vargs m t vres m' ->
...
Hypothesis do_builtin_function_sound:
  ...
  builtin_functions_sem id sg (fun _ => True) ge vargs m t vres m' /\ possible_trace w t w'.
Hypothesis do_builtin_function_complete:
  forall id sg ge vargs m t vres m' w w',
  builtin_functions_sem id sg (fun _ => True) ge vargs m t vres m' ->
  possible_trace w t w' ->
  do_builtin_function id sg ge w vargs m = Some(w', t, vres, m').
...
Hypothesis do_inline_assembly_sound:
  ...
  inline_assembly_sem txt (fun _ => True) ge vargs m t vres m' /\ possible_trace w t w'.

Hypothesis do_inline_assembly_complete:
  forall txt ge vargs m t vres m' w w',
  inline_assembly_sem txt (fun _ => True) ge vargs m t vres m' ->
  possible_trace w t w' ->
  do_inline_assembly txt ge w vargs m = Some(w', t, vres, m').
...
Lemma do_ef_external_sound:
  ...
  external_call ef (fun _ => True) ge vargs m t vres m' /\ possible_trace w t w'.
Proof with try congruence.
  intros until m'.

  assert (VLOAD: forall chunk vargs,
    do_ef_volatile_load chunk w vargs m = Some (w', t, vres, m') ->
    volatile_load_sem chunk (fun _ => True) ge vargs m t vres m' /\ possible_trace w t w').
  intros chunk vargs'.
  ...
  assert (VSTORE: forall chunk vargs,
    ...
    volatile_store_sem chunk (fun _ => True) ge vargs m t vres m' /\ possible_trace w t w').
  intros chunk vargs'.
...
Proof.
  ...
  assert (VLOAD: forall chunk vargs,
             volatile_load_sem chunk (fun _ => True) ge vargs m t vres m' ->
             do_ef_volatile_load chunk w vargs m = Some (w', t, vres, m')).
  intros. inv H1; unfold do_ef_volatile_load.
  exploit do_volatile_load_complete; eauto. intros EQ; rewrite EQ; auto.

  assert (VSTORE: forall chunk vargs,
             volatile_store_sem chunk (fun _ => True) ge vargs m t vres m' ->
             do_ef_volatile_store chunk w vargs m = Some (w', t, vres, m')).
...
Proof.
  ...
  exists t; exists v1; exists w'; auto.
  exists v; auto.
  intros; exists BUILTIN_ENABLED; exists vargs; exists t; exists vres; exists m'; exists w'; auto.
Qed.
...
(* builtin *)
  ...
  (* top *)
  destruct (sem_cast_arguments vtl tyargs) as [vargs|] eqn:?... 
  destruct (builtin_is_enabled ef) as [BUILTIN_ENABLED | BUILTIN_DISABLED]...
  ...
  subst x0. exploit do_ef_external_complete; eauto. congruence.
  apply not_invert_ok; simpl; intros; myinv. specialize (H ALLVAL). myinv. contradiction.
  apply not_invert_ok; simpl; intros; myinv. specialize (H ALLVAL). myinv. 
  exploit sem_cast_arguments_complete; eauto. intros [vtl' [A B]]. congruence.
  (* depth *)
