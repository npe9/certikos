...
Proof.
  eapply Genv.genv_next_transf_partial.
  eassumption.
Qed.
...
(** [CompCertX:test-compcert-param-memory] We are now using the new version of the theorem
  downgraded from [Type] to [Prop]. *)
  assert (X: exists tm', Mem.free tm sp 0 (fn_stackspace tf) = Some tm').
...
(* skip seq *)
  monadInv TR. left.
(** [CompCertX:test-compcert-param-extcall] [dependent induction] and type class hypotheses interact badly with each other. We have to revert them away from the context by hand to avoid any interaction. *)
  revert external_calls_ops compiler_config_ops compiler_config.
...
(** [CompCertX:test-compcert-param-extcall] This [intros] cancels the effect of [revert] above. It is necessary for [exploit] because the induction hypothesis requires those previously reverted type class hypotheses. *)
  intros.
...
(* store *)
  ...
  eapply writable_block_genv_next. apply genv_next_preserved.
  eapply WRITABLE. reflexivity.
  econstructor; eauto.
