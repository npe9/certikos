...
Fixpoint transl_globdefs
    (l: list (ident * option (globdef Csyntax.fundef type)))
    (g: generator) : res (list (ident * option (globdef Clight.fundef type))) :=
  match l with
  | nil => OK nil
  | (id, Some (Gfun f)) :: l' =>
      ...
      | Res tf g' _ =>
          do tl' <- transl_globdefs l' g'; OK ((id, Some (Gfun tf)) :: tl')
      end
  | (id, Some (Gvar v)) :: l' =>
      do tl' <- transl_globdefs l' g; OK ((id, Some (Gvar v)) :: tl')
(** [CompCertX:test-compcert-void-symbols] case none *)
  | (id, None) :: l' =>
    do tl' <- transl_globdefs l' g; OK ((id, None) :: tl')
  end.
