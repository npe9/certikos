...
(** [CompCertX:test-compcert-param-memory] TODO: [lred] does not use memory at all, so why keep it? 
  As long as we keep it, it then depends on the type [mem] for
  memory states, which is an implicit argument. To have Coq
  guess the right one, we make [lred] also depend on memory operations.
  But we have to declare it explicitly, as we cannot have
  this definition reuse the existing context. *)
Inductive lred `{memory_model_ops: Mem.MemoryModelOps mem}: expr -> mem -> expr -> mem -> Prop :=
...
(** [CompCertX:test-compcert-param-memory] The state now depends on the type [mem] for
  memory states, which is an implicit argument. To have Coq
  guess the right one, we make [state] also depend on memory operations.   But we have to declare it explicitly, as we cannot have
  this definition reuse the existing context. *)
