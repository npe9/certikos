...
Proof.
  eapply Genv.mge_next with (new_globs := nil).
...
(** [CompCertX:test-compcert-param-memory] Some implicit argument names change under the hood,
 in spite of [Print Implicit]. Probably a bug of Coq 8.4pl1? *)
  apply S. apply tr_paren_val with (a3 := dummy_expr); auto. econstructor; eauto. 
  ...
  rewrite <- Kseqlist_app.
  eapply match_exprstates; eauto.
  apply S. apply tr_paren_effects with (a3 := dummy_expr); auto. econstructor; eauto. 
  apply tr_expr_monotone with tmp2; eauto. auto. auto.
  (* for set *)
  exploit tr_simple_rvalue; eauto. intros [SL [TY EV]].
  subst sl0; simpl Kseqlist.
  ...
  rewrite <- Kseqlist_app.
  eapply match_exprstates; eauto.
  apply S. apply tr_paren_set with (a3 := dummy_expr) (t := sd_temp sd); auto.
  apply tr_paren_set with (a3 := a2) (t := sd_temp sd).
...
(* seqand false *)
  ...
  eapply match_exprstates; eauto.
  apply S. apply tr_paren_set with (a3 := dummy_expr) (t := sd_temp sd); auto.
  apply tr_paren_set with (a3 := a2) (t := sd_temp sd); auto.
  apply tr_expr_monotone with tmp2; eauto. auto. auto.
(* condition *)
  exploit tr_top_leftcontext; eauto. clear H9. 
  intros [dst' [sl1 [sl2 [a' [tmp' [P [Q [R S]]]]]]]].
...
(* assignop *)
  ...
  exploit tr_simple_lvalue. eauto. eapply tr_expr_invariant with (le0 := le) (le'0 := le'). eauto. 
  intros. apply INV. NOTIN. intros [? [? EV1']].
  exploit tr_simple_rvalue. eauto. eapply tr_expr_invariant with (le0 := le) (le'0 := le'). eauto. 
  ...
  intros. apply INV. NOTIN. simpl. intros [SL2 [TY2 EV2]].
  exploit tr_simple_lvalue. eauto. 
    eapply tr_expr_invariant with (le0 := le) (le'0 := PTree.set t v3 le'). eauto. 
