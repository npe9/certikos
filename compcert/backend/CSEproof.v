...
(** [CompCertX:test-compcert-param-memory] We create section [WITHMEM] and associated
 ...
 We also parameterize over [mmatch_inj], the proof that
 matching a concrete memory state with an abstract representation
 yields a memory that injects into itself.  *)
...
(** [CompCertX:test-compcert-param-memory] additional goal: provide the memory model *)
(** [CompCertX:test-compcert-param-extcall] memory model is no longer available directly in the context, it has to be found through [CompilerConfiguration] instead *)
...
Lemma transf_step_correct:
  ...
  forall s1' (MS: match_states s1 s1') (SOUND: sound_state prog rm s1),
...
- (* Istore *)
  ...
  intros; eapply writable_block_genv_next.
  eapply genv_next_preserved; eauto. eauto.
  econstructor; eauto.
  eapply analysis_correct_1; eauto. simpl; auto. 
  unfold transfer; rewrite H.
  inv SOUND.
...
- (* Ibuiltin *)
  ...
  (* [CompCertX:test-compcert-protect-stack-arg] commit c4c7137ef1849c345961e0547093c98c67d74b87 *)
...
Proof.
  intros. inversion H. 
  exploit (funct_ptr_translated rm); eauto. intros [tf [A B]].
...
- apply (symbols_preserved rm); auto.
...
- intros. destruct H0. exploit (transf_step_correct rm); eauto. 
(** [CompCertX:test-compcert-param-memory] additional goal: provide the proof of [mmatch_inj] *)
  intros [s2' [A B]]. exists s2'; split. auto. split. eapply sound_step; eauto. eauto.
Qed.
