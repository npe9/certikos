...
Proof. apply Genv.genv_next_transf. Qed.
...
(* store *)
  ...
  inv ALD. intros. eapply writable_block_genv_next. apply genv_next_preserved. eauto.
  econstructor; eauto.
...
(** [CompCertX:test-compcert-param-memory] We are now using the new version of the theorem
  ...
  assert (X: exists m'', Mem.free m' sp0 0 (fn_stacksize (transf_function f)) = Some m'').
