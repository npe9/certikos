...
(** [CompCertX:test-compcert-undef-destroyed-by-call] The following
[agree_undef_regs_parallel] is required to erase the value of
non-callee-save registers in BOTH Mach and Asm. To this end, we need
the following bunch of lemmata, because Mach.undef_regs and
Asm.undef_regs are not defined using the same recursion scheme
(Asm.undef_regs is tail-recursive, whereas Mach.undef_regs is not.) *)

Lemma undef_regs_other_1:
  forall l r, ~ In r l ->
              forall rs,
              undef_regs l rs r = rs r.
Proof.
  intros. apply undef_regs_other. intros. congruence.
Qed.

Lemma undef_regs_same:
  forall l r rs, (rs r = Vundef \/ In r l) ->
...
Proof.
  ...
  intros.
  eapply IHl.
  destruct (preg_eq r a).
   subst. rewrite Pregmap.gss. auto.
  rewrite Pregmap.gso; eauto.
  assert (a <> r) by congruence.
  tauto.
Qed.

Fixpoint undef_regs' (rl : list preg) (rs : regset) {struct rl} :
  regset :=
  match rl with
  | nil => rs
  | r1 :: rl' => (undef_regs' rl' rs) # r1 <- Vundef
  end.

Lemma undef_regs'_same:
  forall l r, In r l ->
              forall rs, undef_regs' l rs r = Vundef.
Proof.
  induction l; simpl.
   tauto.
  intros.
  destruct (preg_eq a r).
   subst. intros. apply Pregmap.gss.
  destruct H; try contradiction.
  rewrite Pregmap.gso; eauto.
Qed.

Lemma undef_regs'_other:
  forall l r, ~ In r l ->
              forall rs, undef_regs' l rs r = rs r.
Proof.
  induction l; simpl.
   tauto.
  intros.
...
Lemma undef_regs_undef_regs':
  forall l rs r,
    undef_regs l rs r = undef_regs' l rs r.
Proof.
  intros.
  destruct (List.In_dec preg_eq r l).
   rewrite undef_regs'_same; eauto. apply undef_regs_same; auto.
  rewrite undef_regs'_other; auto. apply undef_regs_other_1; auto.
Qed.
...
Proof.
  intros.
  eapply agree_exten.
  2: intros; eapply undef_regs_undef_regs'.
  induction rl; simpl.
   tauto.
  inv IHrl; eauto.
  constructor.
   apply Pregmap.gso. destruct a; simpl; congruence.
  assumption.
  intro.
  destruct (mreg_eq a r).
   subst. rewrite Regmap.gss. constructor.
  rewrite Regmap.gso. rewrite Pregmap.gso. auto.
  intro. exploit preg_of_injective; eauto. congruence.
Qed.
...
(** [CompCertX:test-compcert-param-memory] We create section [WITHMEM] and associated
 ...
 This section has to be isolated from [WITHMEM2] below because
 there are [Hint]s at the top level in between, which must not be
...
(** [CompCertX:test-compcert-param-memory] We create section [WITHMEM] and associated
 contexts to parameterize the proof over the memory model.
 This section has to be isolated from [WITHMEM] above because
 there are [Hint]s at the top level in between, which must not be
 enclosed in sections.  *)
(** [CompCertX:test-compcert-param-extcall] Actually, we also need to parameterize
    over external functions. To this end, we created a [CompilerConfiguration] class
...
(** [CompCertX:test-compcert-protect-stack-arg] The initial stack
pointer is now a parameter of the semantics. For whole programs, it
is [Vzero]. *)

(** [CompCertX:test-compcert-protect-stack-arg] Likewise, the initial
return address is now a parameter of the semantics. For whole
programs, it is [Vzero]. *)

Variables (init_sp init_ra: val) (init_sp_not_vundef: init_sp <> Vundef) (init_ra_not_vundef: init_ra <> Vundef).
