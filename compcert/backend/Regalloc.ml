...
let block_of_RTL_instr i64helpers funsig tyenv = function
  | RTL.Inop s ->
      [Xbranch s]
  ...
  | RTL.Ibuiltin(ef, args, res, s) ->
      let (cargs, cres) = mregs_for_builtin i64helpers ef in
...
let function_of_RTL_function i64helpers f tyenv =
  let xc = PTree.map1 (block_of_RTL_instr i64helpers f.RTL.fn_sig tyenv) f.RTL.fn_code in
...
let regalloc i64helpers f =
  ...
  | OK tyenv ->
      let f2 = function_of_RTL_function i64helpers f1 tyenv in
