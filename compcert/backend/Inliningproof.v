...
(** [CompCertX:test-compcert-param-inlining] We now parameterize over
the function environment for inlining. For whole programs, it will be
instantiated with [funenv_program p]. *)

Section WITHFENV.

Variable fenv: funenv.
Hypothesis TRANSF: transform_partial_program (transf_fundef fenv) prog = OK tprog.
...
(** [CompCertX:test-compcert-param-memory] We are now using the new version of the theorem
  ...
  assert (X: exists m1', Mem.free m'0 sp' 0 (fn_stacksize f') = Some m1').
...
| match_states'_intro
    ...
    (MATCH: match_states fenv m_init s s')
.
...
Proof.
  ...
  exploit step_simulation; eauto. apply funenv_program_compat. destruct 1.
