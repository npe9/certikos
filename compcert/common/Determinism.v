...
Arguments possible_behavior [_] _ _.
...
Arguments sem_deterministic [_] _.
...
Ltac use_step_deterministic :=
  ...
  | [ S1: Step L _ ?t1 _, S2: Step L _ ?t2 _ |- _ ] =>
    destruct (det_step _ L DET _ _ _ _ _ S1 S2) as [EQ1 EQ2]; subst
  end.
...
Definition same_behaviors (beh1 beh2: program_behavior RETVAL) : Prop :=
...
Proof.
  generalize (det_final_nostep _ L DET); intro dfns.
...
Definition world_sem : semantics RETVAL := @Semantics
  RETVAL                                             
  (state L * world)%type
  (funtype L)
  (vartype L)
